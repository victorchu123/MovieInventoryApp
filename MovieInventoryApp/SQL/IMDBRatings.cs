...
namespace SQL
{
    public class IMDBRatings
    {
        ...
        private void UpdateDB(Movie movie)
        {
            ...
            Date release_date = movie.ReleaseDate;
            string genre = movie.Genre;
            Rank imdb_rank = movie.IMDBRank;

            string sql_query = String.Format(
                @"INSERT INTO dbo.Movies 
                (ID, Title, ReleaseDate, Genre, IMDBRank) 
                VALUES ({1}:, {2}:, {3:yyyy}, {4}:, {5}:",
                id, title, release_date, genre, imdb_rank);

            sql_querier.QueryOrderData(sql_query);
        }
